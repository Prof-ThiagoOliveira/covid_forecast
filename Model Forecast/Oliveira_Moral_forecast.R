...
work_data$Time <- as.numeric(work_data$time) -
  min(as.numeric(work_data$time)) + 1
...
work_data  <- work_data %>%
  filter(Time < max(work_data$Time)) %>%
  droplevels()
tail(work_data)
#=======================================================================
...
country_matrix <- cbind(country_matrix,
                        matrix(NA, ncol = n_ahead,
                               nrow = nrow(country_matrix)))
...
data_forecast <- data.frame("Country" = rep(rownames(country_matrix),
                            ...
                            "Lower" = Y_forecast$`2.5%`,
                            "Upper" = Y_forecast$`97.5%`)
...
work_data2$Lower <- work_data2$cases
work_data2$Upper <- work_data2$cases
...
shiny_data <- work_data2[, c("Country", "cases", "time", "Indicator",
                             "Lower", "Upper")]
shiny_data <- rbind(shiny_data,
                    data_forecast[, c("Country", "cases", "time",
